...
export default function CourseCard({ course }) {
  return (
    <div className="card-base group p-6 flex flex-col h-full relative transition-all duration-200 hover:-translate-y-0.5">
      ...
      <p className="text-sm text-ink-600/80 mb-4 flex-1">{course.summary}</p>

      <ul className="space-y-1.5 mb-5">
        {course.highlights.slice(0, 3).map((h, i) => (
          <li key={i} className="flex items-start gap-2 text-sm text-ink-600/80">
      ...
      <div className="flex items-center justify-between border-t border-ink-50 pt-4">
        <div className="text-sm">
          <p className="text-ink-600/60">{course.duration}</p>
